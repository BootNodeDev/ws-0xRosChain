...
contract ZxRosChainNFT is ERC721 {
    ...
    error ZxRosChainMinter__onlyMinter();
    error ZxRosChainMinter__mint_maxSupplyReached();

    /**
     * @dev Sets the Base URI, name and symbol.
     */
    constructor(string memory _uri, uint256 _maxSupply, address _minter) ERC721("0xRosChainNFT", "0xRSC") {
        _baseTokenURI = _uri;
     ...
     */
    modifier onlyMinter() {
        if (msg.sender != MINTER) revert ZxRosChainMinter__onlyMinter();
        _;
    }
